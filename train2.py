...
root_path = "./data/"
train_dataset = HandData(root_path=root_path,
                         label_hand_pose_file_list=['InterHand2.6M_label_hand_pose_train.npy'])
train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)

val_dataset = HandData(root_path=root_path,
...
for epoch in range(num_epochs):
    ...
    for i, (hand_pose) in enumerate(train_dataloader):
        ...
        loss_recon_bias = 0.2 * Loss_rotation_matrix(pred_bias, hand_pose)
        ...
        setting_cov = 1 / 4 * torch.exp(-torch.norm(latent_locs, dim=1)**2 / 2)

        local_covariance = latent_distribution.variance

        # Wass_Local = batch_trace(local_covariance) + latentD*setting_cov - 2*torch.sqrt(torch.sum(local_covariance, dim=-1)/(torch.add(setting_cov,1e-8)))

        Wass_Local = torch.tensor(0.0, dtype=torch.float32)
        Wass_Local = 0.1*Wass_Local.mean()
        ...
        loss = loss_recon + loss_recon_bias + loss_recon_similarity + KL_locs_global + Wass_Local
        ...
        writer.add_scalar("train/KL_local_ditribution", Wass_Local, curr_step)
        writer.flush()

        if (i % 10) == 0:
            print(
                'Epoch [{}/{}], Step [{}/{}], loss_recon: {:.4f}, loss_recon_bias: {:.4f}, loss_recon_similarity: {:.4f}, KL_locs: {:.4f}, Wass_Local: {:.4f}, spend time: {:.4f}'
                .format(epoch + 1, num_epochs, i + 1, total_step, loss_recon.item(), loss_recon_bias.item(),
                        loss_recon_similarity.item(), KL_locs_global.item(), Wass_Local.item(),
                        time.time() - start))
            start = time.time()
    ...
    with torch.no_grad():
        ...
        for i, (hand_pose) in enumerate(val_dataloader):
            ...
            val_KL_local_ditribution.append(Wass_Local.item())
        ...
        print(
            'Epoch [{}/{}], loss_recon: {:.4f}, loss_recon_bias: {:.4f}, loss_recon_similarity: {:.4f}, KL_locs: {:.4f}, Wass_Local: {:.4f}, time: {:.4f}'
